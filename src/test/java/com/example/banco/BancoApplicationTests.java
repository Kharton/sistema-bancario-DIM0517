...
class BancoApplicationTests {
	 ...
	 *
	 * ▪ Consultar Conta – Obs. Diferentes testes para cada tipo de conta 1 = Normal 2 =
	 * Bonificada 3 = Poupança
	 */
	...
	public void cadastrarContaNormal() {
		...
		var userAction = userService.registerUser(user2.getNumUser(), user2.getType(),
				user2.getSaldo());
		user2.setSaldo(0);
	...
	public void creditarValorPositivoService() {
		...
		Assertions.assertThat(userAction.getSaldo())
				.isEqualTo(user.getSaldo() + user.getSaldo());
	}

	@Test
	public void creditarValorNegativoService() {
		user.setSaldo(-50);
		Assertions.assertThatExceptionOfType(NegativeValueException.class).isThrownBy(
				() -> userService.creditUser(user.getNumUser(), user.getSaldo()));
	}
	...
	void debitarValorMaiorChequeService() {
		...
		Assertions.assertThatExceptionOfType(MinFundsExceededException.class).isThrownBy(
				() -> userService.debitUser(user.getNumUser(), user.getSaldo()));
	}
	...
	void debitarValorMaiorTipo2Service() {
		...
		repository.save(user);
		user.setSaldo(user.getSaldo() + 1);
		Assertions.assertThatExceptionOfType(InsufficientFundsException.class).isThrownBy(
				() -> userService.debitUser(user.getNumUser(), user.getSaldo()));
	}
	...
	void transferirEntreContasService() {
		...
		user2 = userService.transfer(user2.getNumUser(), user.getNumUser(),
				user2.getSaldo());
	...
	void transferirEntreContasNegativoService() {
		...
		Assertions.assertThatExceptionOfType(InsufficientFundsException.class)
				.isThrownBy(() -> userService.transfer(user2.getNumUser(),
						user.getNumUser(), user2.getSaldo() + 1001));
	}
